...
namespace MapGeneratorTool
{
	...
	struct NoiseData 
	{
		...
		NoiseData(int s, int oct, float f, float scl, float lac, float fractalgain) :
			seed(s), octaves(oct), frequency(f), scale(scl), lacunarity(lac), fractalGain(fractalgain)
		{};
		NoiseData() = default;
	};
	...
	struct LookupMapData
	{
		...
		LookupMapData(NoiseData noiseData, LookupFeatures landSpec, LookupFeatures oceanSpec, unsigned w, unsigned h, float borderThick, float cutOff) :
