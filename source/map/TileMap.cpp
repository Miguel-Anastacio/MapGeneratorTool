...
void TileMap::FloodFillTileMap(const std::vector<mygal::Vector2<double>>& centroids, std::unordered_set<Utils::Color>& colorsUnavailable)
{
	auto width = Width();
	auto height = Height();
	Timer timer;
	...
	for (unsigned y = 0; y < height; y++)
	{
		for (unsigned x = 0; x < width; x++)
		{
			...
			if (!m_tiles[index].visited)
			{
				...
				{
					color = Utils::GetRandomColorNotInSet(m_colors);
